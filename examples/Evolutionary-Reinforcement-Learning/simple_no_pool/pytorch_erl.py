    ...
    def evaluate(self, trials=1):
        ...
        for ep in range(trials):
            ...
            while not done:
                ...
                action_new = np.clip(np.hstack((action.reshape(2, 2), np.zeros((2, 1)))).ravel(), -0.5, 0.5)
...
class ERL:
    ...
    def evaluate(self, actor, times=1, episode=1):
        ...
        for i in range(times):
            ...
            while not done:
                ...
                total_steps += 1
                d = np.reshape(np.array(1.0 if done == True else 0.0, dtype=np.float32), (1, 1))
                self.ddpg.add_to_buffer([flatten_observation(obs), action, [r[0]+r[1]], flatten_observation(new_obs)])
                obs = new_obs
                if total_steps > 1500:
                    done = True
            kill_env(env)
    ...
    def mutation(self, actor):
        for target_param in actor.parameters():
            noised = torch.normal(target_param.data, torch.abs(target_param.data * 0.1))
