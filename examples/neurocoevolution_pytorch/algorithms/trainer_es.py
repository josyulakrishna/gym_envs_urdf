...
# wandb.init(project="neurocoevolution_mr_carry_es", entity="josyula", sync_tensorboard=True, config=DEFAULT_CONFIG)
...
class ESTrainer(EATrainer):
    ...
    def step(self):
        ...
        for i in range(len(weights_mutations1)):
            fitness_player1 += [fitness_pool.starmap(self.worker_class.evaluate_team_fitness, zip(weights_mutations1[i], [self.player2_hof_best[0]]*self.n))]

        #player2 fitness evaluation
        fitness_player2 = []
        for i in range(len(weights_mutations2)):
            fitness_player2 += [fitness_pool.starmap(self.worker_class.evaluate_team_fitness, zip( [self.player1_hof_best[0]]*self.n, weights_mutations2[i]))]
        ...
        total_fitness_player1 = np.sum(np.dot(fitness_player1, epsilons_mutations1.T), axis=1)/(len(fitness_player1)*len(fitness_player1[0]))
        f1 = torch.FloatTensor([(alpha / sigma) * np.sum(total_fitness_player1)])
        for key in self.population_player1[0].keys():
            self.population_player1[0][key] = self.population_player1[0][key] + f1

        #update weights of player2
        fitness_player2 = np.array(fitness_player2)
        epsilons_mutations2 = np.array(epsilons_mutations2)

        total_fitness_player2 = np.sum(np.dot(fitness_player2, epsilons_mutations2.T), axis=0)/(len(fitness_player2)*len(fitness_player2[0]))
        ...
        for i in range(len(self.player2_hof)):
            self.player2_hof.append(weights_mutations2[i][best_weights_player1[i]])
