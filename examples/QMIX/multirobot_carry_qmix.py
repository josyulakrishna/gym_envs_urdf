...
parser.add_argument('--max_episode_len', type=int, default=10000, help='maximum episode length')
...
parser.add_argument('--replay_size', type=int, default=5000, help='maximum number of episodes of replay buffer')
...
memory = ReplayMemory(args.replay_size, args.max_episode_len, env.n_, obs_shape, action_shape)

trainer = AgentsTrainer(env.n_, obs_shape, action_shape, args)
total_numsteps = 0
updates = 0
t_start = time.time()
...
for i_episode in itertools.count(1):
    ...
    while not done:
        ...
        action_act = np.hstack((deepcopy(action_list), np.zeros((len(action_list),1)))).ravel()
